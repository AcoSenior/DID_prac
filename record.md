...
1. 재정립된 db 스키마를 코드화 (외래키 기능은 일단 사용하지 않음)

2. 라우터를 일부 분리하고 프론트에서도 요청 보내는 라우터의 uri를 수정)

3. 일부 유실된 기능 (사용자 정보 요청 선택) 복구\
...
- 사용자가 이메일, 패스워드를 입력후, 요청을 보내면 그걸 가지고 블록체인과 상호작용해 올바른 사용자인지 판단
- 맞는 사용자인 경우 쿠키를 준다.
  이 쿠키에 어떤 정보가 담겨있는지는 일단 머지하고 생각해보자.
...
> 블록체인에 접근해 해당 hash(id+pw)를 가진 user 객체가 있는지 확인
> 있다면 그 객체의 정보를 쿠키로 만들어서 브라우저에 준다.
> 이렇게 받은 쿠키는 사용자의 다양한 정보를 불러오는데 사용할 수 있다.
> 이를 위해 사용하는 것이 getServerSideProps인데,
> 페이지 렌더링 전 데이터를 먼저 받아오는 용도로 사용하는 것 같다.
> (oauth_server/front/pages/mypage.jsx 참조)
> 잘 된건지 모르겠네..

---

회원 정보의 수정, 삭제 기능을 (컨트랙트와 연동해서) 만들어보자.

삭제는 뭐 어려울 것 없고

수정이 문젠데, 특히 그 중에서도 비밀번호를 바꿀 경우

엄청나게 많은 db에 접근해 데이터를 수정해야 한다.

이게 나는 별로 좋은 방법같지가 않은데..

아무튼 비밀번호라는 친구는 조금 특별하다.

1. 삭제

삭제를 할 경우도 마찬가지, 해당 회원의 모든 정보를 모든 db에서 제거해야한다.

지금이야 oauthdb만 건들고 있지만 나중엔 localDB로도 영향을 줘야할텐데..

머 사실 이건 그냥 다 지워버리면 되는거고 수도 적어서 괜찮긴 하다..

`` 9/6

1. 회원 탈퇴 / 데이터 삭제 (contd..)

- 하다보니 사용자의 hash를 db에 저장하는 것도 (이게 털리면 다 털리니까),

변할 수 있는 값인 hash를 식별자로 사용하는 것도 좋은 방법이 아니라는 생각이 들어

이를 수정하기로 했다.

> userInfo db에서 hash를 삭제, 식별자는 이메일로 대체하기로 했다.

> 회원 탈퇴 기능의 경우, 블록체인에서 사용자를 삭제하는 것 이외에도

여러 db와 상호작용해서 데이터를 삭제해줄 필요가 있다.

이 db에서 관련 데이터를 모두 삭제 후, 블록체인에서 사용자 데이터를 지우는게 좋다.

일단 여기까진 된 듯

2. 회원 수정 기능

2-1. 사용자의 비밀번호는 어떻게 처리할 것인가..

여전히 블록체인 상에선 hash(id+pw)를 사용하므로

서버의 db에서 더 이상 hash를 사용하지 않는다고 해도 이는 특별 취급이 필요하다.

2-2. 나머지 개인 정보 수정

얘는 그냥 수정하면 된다. 특이 사항이 생각나지 않음.
